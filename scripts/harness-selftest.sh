...
# either literal flag as one contiguous token — the CI content-scan guards
# (see .github/workflows/ci.yml) match src/ and scripts/ for exactly those
# substrings, and these are probe values/regex fragments (proving the
# detection pattern would catch them), never actual invocations/references.
REMOTE_FLAG="--""remote"
DBPROD_FLAG="DB_""PROD"
for probe in "wrangler d1 migrations apply DB $REMOTE_FLAG" "const x = $DBPROD_FLAG" "const r = 39.49" "it.skip('x', () => {})"; do
  echo "$probe" > "$tmp/probe.ts"
  cp "$tmp/probe.ts" ./_probe.ts && git add ./_probe.ts >/dev/null 2>&1
  if git diff --cached ./_probe.ts | grep -qE "^\+.*($REMOTE_FLAG|$DBPROD_FLAG|39\.49|\.skip\()"; then
