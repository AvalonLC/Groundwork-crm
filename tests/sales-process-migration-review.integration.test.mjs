...
CREATE TABLE sales_stage_assignments (id TEXT PRIMARY KEY, company_id TEXT, opportunity_id TEXT, process_version_id TEXT, stage_id TEXT, classification TEXT, outcome_type TEXT, internal_status_id TEXT DEFAULT '', assigned_by TEXT, UNIQUE(company_id,opportunity_id,process_version_id));
...
test('readiness rejects pending, stale, cross-tenant, terminal, and nonterminal data', async () => {
  ...
  await run("UPDATE opportunities SET assigned_to_rep_id='manager-a' WHERE id='nonterminal' AND company_id='company-a'");
  ...
  assert.ok(readiness.issues.some(issue => issue.code === 'stale_assignment' && issue.opportunity_id === 'nonterminal'));
...
test('a failed publication batch rolls back every lifecycle and data change', async () => {
  ...
  await run("UPDATE opportunities SET assigned_to_rep_id='rep-a' WHERE id='nonterminal'");
  const proposed = await body(await request(tokens.admin, '/api/sales-process/migration/propose', { method: 'POST', body: JSON.stringify({ process_version_id: 'version-a' }) }));
