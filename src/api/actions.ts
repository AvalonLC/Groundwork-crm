...
export type ActionsBindings = { FINANCE_DB: D1Database };
...
actionsRouter.post("/", async (c) => {
  const body = await c.req.json<{
    id: string; tenant_id: string; verb: string; owner_id?: string; sla_due?: string;
  ...
  if (!body.id || !body.tenant_id) return c.json({ error: "id and tenant_id are required" }, 400);

  await insertActionItem(c.env.FINANCE_DB, {
    id: body.id,
...
actionsRouter.get("/", async (c) => {
  ...
  if (!tenantId) return c.json({ error: "tenant_id query param is required" }, 400);
  const verb = c.req.query("verb") as ActionVerb | undefined;
  if (verb && !VALID_VERBS.includes(verb)) {
    return c.json({ error: `verb must be one of: ${VALID_VERBS.join(", ")}` }, 400);
  }
  const items = await getOpenActionItems(c.env.FINANCE_DB, tenantId, verb);
  return c.json({ items });
});
...
actionsRouter.post("/:id/resolve", async (c) => {
  const tenantId = c.req.query("tenant_id");
  if (!tenantId) return c.json({ error: "tenant_id query param is required" }, 400);
  await resolveActionItem(c.env.FINANCE_DB, tenantId, c.req.param("id"));
