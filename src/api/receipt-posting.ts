import {
  getReceiptForPosting, receiptHasPostedLedgerLine, markReceiptPostedStatement,
  postDirectCostLedgerLineStatement,
} from "../db/repos";
 ...
 *  6. Create the ledger entry transactionally (db.batch, step 7 below).
 *  7. Mark the receipt posted exactly once (same batch, same atomicity).
...
export async function postApprovedReceiptToLedger(
  ...
  // Steps 6-7: create the ledger entry AND mark the receipt posted in one
  // atomic batch — either both writes land or neither does, so a receipt
  // can never end up "posted" with no ledger line, or vice versa.
  const insertStmt = postDirectCostLedgerLineStatement(db, {
  ...
  const markPostedStmt = markReceiptPostedStatement(db, companyId, receiptId);

  const results = await db.batch([insertStmt, markPostedStmt]);
  const ledgerResult = results[0] as D1Result;
  const receiptResult = results[1] as D1Result;
  const ledgerChanges = ledgerResult.meta.changes ?? 0;
  const receiptChanges = (receiptResult.meta.changes ?? 0) > 0;

  if (ledgerChanges === 0 || !receiptChanges) {
    // A concurrent request won the race between our read (step 5) and this
    // write — the posted_at IS NULL guard inside markReceiptPostedStatement
    // caught it. Step 10: return a safe conflict response, not an error.
    return { success: false, reason: "already_posted" };
  }

  // Step 8: source attachment (r2_key/content_hash) is never touched above.

  const ledgerLineId = ledgerResult.meta.last_row_id as number;
