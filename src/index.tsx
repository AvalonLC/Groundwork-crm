...
// Bindings/Variables live in src/env.ts so src/portal.tsx can share them
// without importing this file back. See that module for why.
import type { Bindings, Variables } from './env'

/**
 * What every handler and middleware in this file receives.
 *
 * Generic over the route path on purpose. Hono derives `c.req.param('id')` as
 * `string` from the path literal a route was registered with, and pinning this
 * to the default lost that everywhere — a middleware typed with a concrete
 * Context degrades the inference for every route it guards, turning 26 safe
 * `c.req.param('id')` reads into `string | undefined`. Threading the path
 * parameter through keeps both: typed env/vars AND typed params.
 */
type AppContext<P extends string = string> = Context<{ Bindings: Bindings; Variables: Variables }, P>
...
// `json`/`err` are exported so standalone routers mounted elsewhere (e.g.
// src/ai/lead-import-routes.ts) return the exact same response envelope as
// every handler in this file, rather than inventing a second shape.
export function json(c: any, data: any, status = 200) {
...
async function verifyPin(pin: string, stored: string): Promise<boolean> {
  ...
  // A length check does not narrow a string[] for the compiler, and reaching for
  // `!` here would suppress the one place in this file where a wrong answer is a
  // security bug. Read them out explicitly instead.
...
async function requireAuth<P extends string>(c: AppContext<P>, next: Next) {
  const token = getCookie(c, 'avalon_session')
  if (!token) return err(c, 'Unauthorized', 401)
  const row = await c.env.DB.prepare(`
    SELECT r.id as rep_id, r.company_id, r.role, r.is_super_admin, r.can_view_compensation, s.updated_at as session_at
    FROM settings s
    JOIN reps r ON r.id = s.value
    WHERE s.key = ? AND r.active = 1 LIMIT 1
  `).bind(`session_${token}`).first<{ rep_id: string; company_id: string; role: string; is_super_admin: number; can_view_compensation: number; session_at: string }>()
  ...
  // Migration 0074. Carried on the context so any route can gate on it without
  // a second read — the read rule and the write rule for compensation are the
  // same rule and both need it.
  c.set('canViewCompensation', row.can_view_compensation === 1)
  await next()
}
...
async function requireAuthFinance<P extends string>(c: AppContext<P>, next: Next) {
...
async function resolveSalesOpportunityStage(db: D1Database, companyId: string, opportunityId: string) {
...
app.post('/api/sales-process/drafts/from-current-pipeline', requireAuth, async (c) => {
  ...
  const firstStage = draftStages[0]
  // .some() on an empty array is false, so this ran exactly when there was no
  // stage to mark — an empty `labels` threw here rather than producing a process.
  if (firstStage && !draftStages.some(s => s.semantic === 'intake')) firstStage.semantic = 'intake'
...
app.get('/api/sales-process/versions/:versionId/preview', requireAuth, async (c) => {
  ...
  const methods: { automatic: number; manual: number; unknown: number } = { automatic: 0, manual: 0, unknown: 0 }
...
app.on(['PUT', 'POST'], ['/api/opportunities/:id/sales-stage', '/api/opportunities/:id/stage-transition'], requireAuth, async (c) => {
  ...
  const overrideResponse = (message: string, status: ContentfulStatusCode, details: any = {}) => c.json({ ok: false, error: message, data: {
...
app.post('/api/auth/invite', requireAuth, async (c) => {
  ...
  {
    const _b: any = await c.req.raw.clone().json().catch(() => ({}))
...
app.get('/api/admin/ai', requireSuperAdmin, async (c) => {
  ...
  return json(c, {
    ...
    companies: (companies.results as any[]).map((co: any) => ({
      ...
      ai_plan: aiPlanOf(planMap[co.id]),
      ai_cap: (() => {
        const cc = capMap[co.id]
        return (cc !== undefined && cc !== '' && isFinite(Number(cc))) ? Number(cc) : AI_PLAN_CAPS[aiPlanOf(planMap[co.id])]
      })(),
...
app.get('/api/admin/stats', requireSuperAdmin, async (c) => {
  ...
  return json(c, {
    companies: (companies?.results?.[0] as any)?.n ?? 0,
    reps:      (reps?.results?.[0] as any)?.n ?? 0,
    opps:      (opps?.results?.[0] as any)?.n ?? 0
  })
})
...
app.get('/api/crews', requireAuth, async (c) => {
  ...
  const result = (crews.results || []).map((cr: any) => {
    const members = cr.member_list ? cr.member_list.split(',').map((m: string) => {
      ...
      return { repId, crewRole, name: (repId ? repMap[repId]?.name : null) || repId }
    }) : []
...
app.post('/api/estimates/:id/accept', requireAuth, async (c) => {
  const db = c.env.DB as D1Database
  // `companyId` is not a variable in this handler — every other line here reads
  // c.var.companyId. This threw ReferenceError on every acceptance, i.e. a 500
  // on the flow that turns an estimate into work. Introduced by #57 and invisible
  // because src/index.tsx was outside the typecheck gate at the time.
  await lockEstimateLaborRate(db, c.var.companyId as string, c.req.param('id'), 'accepted')
...
//
// ensureAiSchema / _aiCreds / _aiParseJson / _aiChatJson / _logAiUsage /
// _aiQuota / _aiQuotaGate / AI_PLAN_CAPS / aiPlanOf moved to src/ai/infra.ts
// (imported at the top of this file) so a standalone router mounted the same
// way as marketingRouter/ratesRouter (see src/ai/lead-import-routes.ts) can
// import them without an import cycle back into this file. No behavior
// changed — every call site below is unchanged.
// ══════════════════════════════════════════════════════════════════════════════
...
app.post('/api/calendar/sync', requireAuth, async (c) => {
  ...
  for (const m of (ended as any[])) {
    ...
    if (!dupe) {
      ...
      // An unmapped meeting type is not an error, it is just a meeting we have no
      // follow-up rule for. Reading rule.title on it threw and took the rest of
      // the automation run with it.
      if (!rule) continue
 ...
 */
/**
 * Record a lifecycle event. Never throws — an audit write must not be able to
 * fail the operation it is describing, and a void that half-succeeded because
 * its audit row failed would be worse than a void with no audit row.
 */
async function logInvoiceLifecycle(
  db: D1Database,
  row: {
    companyId: string; invoiceId: string; event: string;
    reason?: string | null; refusalCode?: string | null;
  ...
  try {
    await db.prepare(
      `INSERT INTO invoice_lifecycle_events
         (id, company_id, invoice_id, event, reason, refusal_code, actor_id, actor_role)
       VALUES (?,?,?,?,?,?,?,?)`
    ).bind(
      `ile_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
      row.companyId, row.invoiceId, row.event,
      row.reason ?? null, row.refusalCode ?? null,
      row.actorId ?? null, row.actorRole ?? null,
    ).run()
  } catch (_) { /* audit is best-effort; see the comment above */ }
}
  ...
  if (String(b?.status || '') === 'draft') {
    ...
    if (!verdict.ok) {
      await logInvoiceLifecycle(db, {
        companyId, invoiceId: id, event: 'draft_refused',
        reason: verdict.reason, refusalCode: verdict.code,
 ...
 * the record from any other edit, and carrying no explanation. An invoice that
 * has touched money can no longer be deleted (see canHardDelete), so voiding is
 * now the primary way a bad invoice leaves circulation, and "why" has to
 * survive with it or the status change is unattributable months later.
 */
app.post('/api/invoices/:id/void', requireAuth, async (c) => {
  if (!canInvoice(c.var.role as string, 'manage', { isSuperAdmin: c.var.isSuperAdmin as boolean })) return err(c, 'Voiding invoices is limited to admin and office manager', 403)
  const companyId = c.var.companyId as string
  const db = c.env.DB as D1Database
  const id = c.req.param('id')
  const body = await c.req.json().catch(() => ({})) as any
  const reason = String(body?.reason ?? '').trim()
  if (!reason) return c.json({ error: 'A reason is required to void an invoice' }, 400)
  if (reason.length > 500) return c.json({ error: 'Reason must be 500 characters or fewer' }, 400)

  const cur: any = await db.prepare(
    `SELECT status, voided_at FROM invoices WHERE id=? AND company_id=? LIMIT 1`
  ).bind(id, companyId).first()
  if (!cur) return c.json({ error: 'Not found' }, 404)
  if (String(cur.status || '') === 'void') {
    return c.json({ error: 'This invoice is already voided', code: 'voided' }, 409)
  }

  const res = await db.prepare(
    `UPDATE invoices SET status='void', voided_at=datetime('now'), void_reason=?, updated_at=datetime('now')
      WHERE id=? AND company_id=? AND status != 'void'`
  ).bind(reason, id, companyId).run()
  if (!res.meta.changes) return c.json({ error: 'Invoice could not be voided' }, 409)
...
app.delete('/api/invoices/:id', requireAuth, async (c) => {
  ...
  if (!verdict.ok) {
    await logInvoiceLifecycle(db, {
      companyId, invoiceId: id, event: 'delete_refused',
      reason: verdict.reason, refusalCode: verdict.code,
      actorId: c.var.repId as string, actorRole: c.var.role as string,
    })
    return c.json({ error: verdict.reason, code: verdict.code }, 409)
...
app.post('/api/invoices/from-estimate/:estimateId', requireAuth, async (c) => {
  ...
  `).bind(id, companyId, invoiceNumber, est.id, est.client_id||'', est.client_name||'',
    ...
    // est.estimate_number is not a column on estimates -- it is est_number -- so
    // this fallback resolved to undefined and an untitled estimate produced the
    // literal string "Invoice for undefined" on the customer's invoice.
    `Invoice for ${est.title || est.est_number || 'estimate'}`, 'draft',
    ...
    // est.notes is not a column either. The real ones are internal_notes and
    // customer_notes -- and only the customer-facing one belongs on an invoice,
    // so this deliberately does NOT fall back to internal_notes.
    JSON.stringify(lineItems), est.customer_notes||'', portalToken
  ).run()
...
app.post('/api/plan-visits/:id/complete', requireAuth, async (c) => {
  ...
  const now = new Date().toISOString()
  const today = now.split('T')[0] ?? now.slice(0, 10)
...
app.get('/estimates/portal/:token', async (c) => {
  ...
  const STATUS_LABELS: Record<string, string> = { draft:'Draft', sent:'Sent', approved:'Approved ✓', declined:'Declined', expired:'Expired' }
  const statusLabel = STATUS_LABELS[String(est.status)] || est.status
...
app.post('/api/stripe/webhook', async (c) => {
  const stripeKey = (c.env as any).STRIPE_SECRET_KEY as string | undefined
  // Two destinations point at this one URL — Stripe scopes platform-context and
  // connected-account events separately — and each has its own signing secret.
  const webhookSecret = [
    (c.env as any).STRIPE_WEBHOOK_SECRET as string | undefined,
    (c.env as any).STRIPE_CONNECT_WEBHOOK_SECRET as string | undefined,
  ]
  const body = await c.req.text()
  const sig  = c.req.header('stripe-signature')

  // This endpoint used to verify nothing. The comment here said "simplified
  // check for edge / In production, use proper Stripe webhook verification
  // library" and there was no check at all, simplified or otherwise — it parsed
  // whatever JSON it was sent and acted on it. Anyone who could guess an invoice
  // id and a company id could mark an invoice paid, and both appear in URLs the
  // customer already has.
  //
  // Fails closed when STRIPE_WEBHOOK_SECRET is unset, deliberately: Stripe
  // retries a non-2xx for up to three days, so events arriving before the secret
  // is configured are redelivered afterwards rather than lost, and that is a
  // better trade than leaving the door open. See src/api/stripe_signature.ts.
  const verified = await verifyStripeSignature(body, sig, webhookSecret)
  if (!verified.ok) {
    console.log('stripe webhook rejected:', verified.reason)
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (invoiceId && companyId && amountPaid) {
        ...
        if (inv) {
          ...
          // Idempotency, and it has to come FIRST.
          //
          // The insert below used to write `method` and `paid_at`, neither of
          // which is a column on payments (they are `payment_method`, and there
          // is no paid_at). INSERT OR IGNORE suppresses constraint violations,
          // not "no such column", so it threw on every completed checkout and
          // the handler's catch returned 400 to Stripe.
          //
          // Stripe retries a 400. The invoice UPDATE ran BEFORE the throw, so
          // every retry credited the invoice again — amount_paid climbing by the
          // full amount each time, with no payments row to reconcile it against.
          // Fixing the column names alone would have left that, because the id
          // was `py_${Date.now()}`: a fresh id per retry, so OR IGNORE would
          // never have matched.
          //
          // The id is derived from the payment intent instead. One intent, one
          // payment row, and a retry finds it and does nothing.
          const intentId = String(session.payment_intent || '')
          const pymtId = intentId ? `py_${intentId}` : `py_sess_${session.id}`
          const alreadyRecorded = await db.prepare(`SELECT id FROM payments WHERE id=? AND company_id=?`)
          ...
          if (!alreadyRecorded) {
            ...
            // Same column list and 'succeeded' status as the five other payments
            // inserts in this repo. net_amount is the gross: Stripe's fee is not
            // on the session object, it is on the charge's balance transaction,
            // and inventing a percentage here would put a made-up number in a
            // money column. Gross until reconciled.
            await db.prepare(
              `INSERT INTO payments (id, company_id, invoice_id, client_id, amount, amount_cents, net_amount, net_amount_cents, status, payment_method, stripe_payment_intent_id, description, invoice_number, created_at, updated_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'),datetime('now'))`
            ).bind(pymtId, companyId, invoiceId, inv.client_id || null,
              amountPaid / 100, amountPaid, amountPaid / 100, amountPaid,
              'succeeded', 'card', intentId, 'Stripe Checkout', inv.invoice_number || '').run()
            if (newStatus === 'paid') {
              await markOpportunityCollectedFromInvoice(db, companyId, invoiceId, inv.estimate_id)
            }
          }
    ...
    //
    // Everything below this point was unhandled. A refund left the invoice
    // reading "paid", a chargeback did nothing, and a failed payment was
    // silent — while Stripe, which is the actual system of record for the
    ...
    if (effect.kind === 'refund') {
      ...
      const paid: any = await db.prepare(
        `SELECT invoice_id, company_id, amount_cents FROM payments
          WHERE stripe_payment_intent_id = ? AND amount_cents > 0 LIMIT 1`,
      ).bind(effect.payment_intent_id).first()

      if (paid?.invoice_id) {
        const inv: any = await db.prepare(
          `SELECT total_cents, status, invoice_number, client_id FROM invoices WHERE id=? AND company_id=? LIMIT 1`,
        ).bind(paid.invoice_id, paid.company_id).first()

        if (inv) {
          // Write the movement first, then derive the invoice from the ledger.
          //
          // Reconciling the invoice against the EVENT's running total looked
          // right until a replay: Stripe redelivers old events after new ones,
          // and a replayed "20,000 refunded" moved a fully-refunded invoice back
          // to partial. The ledger cannot be moved backwards that way, because a
          // replay adds nothing to it.
          const priorRefunded: any = await db.prepare(
            `SELECT COALESCE(SUM(-amount_cents), 0) AS c FROM payments
              WHERE stripe_payment_intent_id = ? AND amount_cents < 0`,
          ).bind(effect.payment_intent_id).first()

          const delta = refundDelta(
            Number(paid.amount_cents || 0), effect.amount_cents, Number(priorRefunded?.c || 0),
          )

          if (delta > 0) {
            // A negative payments row rather than a refunded_cents column: the
            // refund is a movement, and the ledger is where movements live. The
            // id comes from Stripe's refund id so a redelivery is a no-op.
            await db.prepare(
              ...
              `py_refund_${effect.refund_id}`, paid.company_id, paid.invoice_id, inv.client_id || null,
              -delta / 100, -delta, -delta / 100, -delta,
              'refunded', 'card', effect.payment_intent_id, 'Stripe refund', inv.invoice_number || '',
            ).run()
          }
          ...
          await db.prepare(
            `UPDATE invoices SET amount_paid=?, amount_paid_cents=?, status=?, updated_at=datetime('now')
              WHERE id=? AND company_id=?`,
          ).bind(nowPaid / 100, nowPaid, invoiceStatusFor(Number(inv.total_cents || 0), nowPaid),
                 paid.invoice_id, paid.company_id).run()
                }
      }
    }
    ...
    if (effect.kind === 'dispute') {
      const paid: any = await db.prepare(
        `SELECT invoice_id, company_id FROM payments
          WHERE stripe_payment_intent_id = ? AND amount_cents > 0 LIMIT 1`,
      ).bind(effect.payment_intent_id).first()

      if (paid?.invoice_id) {
        // The invoice's paid state is deliberately NOT changed. A dispute is a
        // case with a deadline, not a movement: the money may come back. What
        // it needs is to be visible, not to be quietly written off.
        await db.prepare(
          `UPDATE invoices SET dispute_status=?, dispute_id=?, disputed_amount_cents=?,
                               disputed_at=COALESCE(disputed_at, datetime('now')), updated_at=datetime('now')
            WHERE id=? AND company_id=?`,
        ).bind(effect.status || effect.phase, effect.dispute_id, effect.amount_cents,
               paid.invoice_id, paid.company_id).run()
      }
    }
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="/js/premium.css?v=20260914b002">  <style>
...
<body>
  ...
  <script src="/js/platform_core.js?v=20260914b002"></script>
  <script src="/js/client_portal.js?v=20260914b002"></script>  <script>
...
<head>
  ...
  <link rel="stylesheet" href="/js/premium.css?v=20260914b002">
  <link rel="stylesheet" href="/js/styles.css?v=20260914b002">
  <link rel="stylesheet" href="/js/groundwork-design.css?v=20260914b002">
  <link rel="stylesheet" href="/js/finance-shell.css?v=20260914b002">  <style>
    /* ── Nav baseline ───────────────────────────────────────────────────────── */
...
<script src="/js/gw_date.js?v=20260914b002"></script>
<script src="/js/gw-icons.js?v=20260914b002"></script>
<script src="/js/sales-process.js?v=20260914b002"></script>
<script src="/js/richtext.js?v=20260914b002"></script>
<script src="/js/db.js?v=20260914b002"></script>
<script src="/js/data.js?v=20260914b002"></script>
<script src="/js/reps.js?v=20260914b002"></script>
<script src="/js/record-page.js?v=20260914b002"></script>
<script src="/js/academy.js?v=20260914b002"></script>
<script src="/js/task_engine.js?v=20260914b002"></script>
<script src="/js/gw_i18n.js?v=20260914b002"></script>
<script src="/js/app_premium.js?v=20260914b002"></script>
<script src="/js/estimates.js?v=20260914b002"></script>
<script src="/js/multiday.js?v=20260914b002"></script>
<script src="/js/proposals.js?v=20260914b002"></script>
<script src="/js/pricing.js?v=20260914b002"></script>
<script src="/js/invoices.js?v=20260914b002"></script>
<script src="/js/csv_import.js?v=20260914b002"></script>
<script src="/js/onboarding.js?v=20260914b002"></script>
<script src="/js/gw_copilot.js?v=20260914b002"></script>
<script src="/js/groundwork_ai.js?v=20260914b002"></script>
<script src="/js/recurring_plans.js?v=20260914b002"></script>
<script src="/js/reviews.js?v=20260914b002"></script>
<script src="/js/stripe.js?v=20260914b002"></script>
<script src="/js/email.js?v=20260914b002"></script>
<script src="/js/notifications.js?v=20260914b002"></script>
<script src="/js/integrations.js?v=20260914b002"></script>
<script src="/js/sms.js?v=20260914b002"></script>
<script src="/js/calendar_sync.js?v=20260914b002"></script>
<script src="/js/ai_followup.js?v=20260914b002"></script>
<script src="/js/user_management.js?v=20260914b002"></script>
<script src="/js/platform_admin.js?v=20260914b002"></script>
<script src="/js/time_tracker.js?v=20260914b002"></script>
<script src="/js/field_workday.js?v=20260914b002"></script>
<script src="/js/platform_core.js?v=20260914b002"></script>
<script src="/js/approval_engine.js?v=20260914b002"></script>
<script src="/js/automation_engine.js?v=20260914b002"></script>
<script src="/js/client_portal.js?v=20260914b002"></script>
<script src="/js/field_mode.js?v=20260914b002"></script>
<script src="/js/assets_hub.js?v=20260914b002"></script><script src="/js/marketing.js?v=20260914b002"></script><script>
