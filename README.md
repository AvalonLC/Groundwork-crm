...
### Frontend
- **Sales Process Builder** view in `public/js/app_premium.js`, surfaced as the second Sales workspace tab ("Sales Process") plus an admin-only "Customize Stages" shortcut on the Pipeline board header. Start screen offers "Use my current pipeline" (imports live stage labels via `POST /api/sales-process/drafts/from-current-pipeline`) or a Groundwork template. Editing is a 3-step wizard (Design Stages / Move Your Leads / Go Live) with an Advanced tools dropdown for internal statuses, requirements, checklists, guides, automations, email templates, Academy and AI panels. Post-publish the client refreshes `window._gwPipelineStages` from the publish response; post-rollback it re-fetches `DB.pipelineStages.list()`.
