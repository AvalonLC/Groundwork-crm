...
| Sales Process Builder | Complete | Optimistic mutations cover stages, statuses, requirements, guides, resources, automations, Academy associations, transitions, and outcomes | Field-specific controls cover overview, stages, statuses, qualification, checklists, guides, automations, email, Academy, AI review, versions, validation, preview, and mapping | Two-company API lifecycle plus mobile DOM add, duplicate, reorder, role, stale-save, occupied-stage, and published-immutability proof | Complete |
| Needs Restaging workspace | Complete | Inventory and review endpoints expose activity, estimates, appointments, value, and notes | Filterable review, individual Save and Next, bulk approval, counts, reconciliation, and responsive layout | Mobile DOM proof covers permanent original text, all review fields, filters, neutral unknown selection, Save and Next, high-confidence bulk approval, and snapshot blocking | Complete |
...
| AI Process Assistant | Complete | Guided interview generates the full draft recommendation catalog; actionable acceptance requires an optimistic component mutation and advisory acceptance requires the current revision | Side-by-side review, guided interview, and individual accept/reject or acknowledge controls | Catalog generation, role, tenant, unsafe-type, stale revision, no-operational-side-effect, and optimistic acceptance tests pass | Complete |
| Snapshot, preview, approval, publication | Complete | Company-scoped snapshot capture, drift-safe approval, transition samples, impact preview, readiness gates, and atomic publication | Builder renders pipeline order, impact totals, mapping methods, surfaces, transitions, gate issues, explicit approval, and failure preservation | Backend failure/success lifecycle and mobile DOM readiness, gate, confirmation, role, impact, and refresh proof | Complete |
| Rollback | Complete | Reactivates prior version and restores prior stage, outcome, and classification while retaining history | Authorized publication history includes explicit rollback confirmation and refresh | Backend restoration and mobile DOM role, confirmation, preservation messaging, API invocation, and refresh proof | Complete |
| Avalon inventory and draft | Complete | Inventory foundation | Review UI foundation | Authenticated Avalon review and leadership approval are unavailable | Blocked by explicit approval gates |
