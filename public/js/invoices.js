...
function _invRenderList(invoices) {
  ...
  // the bar has to be recounted here or it keeps claiming a selection the new
  // rows no longer contain.
...
/**
 * Of a selection, the invoices a card charge would actually be run against.
 *
 * One definition, used by BOTH the bar's count and the run itself. It was
 * written out twice — once to size and disable the button, once to decide what
 * to charge — and two copies of the rule that decides whose card gets billed is
 * one copy too many. Drift there does not throw; the button just says 3 and
 * charges 5.
 *
...
function _invRenderBulkBar() {
  ...
  const chargeable = _invChargeable(picked);
  bar.hidden = false;
  bar.innerHTML = `
...
window._invBulkCharge = async function() {
  ...
  const picked = _invChargeable(_invPicked());
...
/**
 * Turn the bulk-edit form into a PUT body.
 *
 * Blank means LEAVE ALONE, and that is the whole rule. Every field is opt-in, so
 * nothing is overwritten because a form defaulted to something — an empty due
 * date must not clear the due date on nine invoices, and an untouched status
 * select must not push them all back to draft.
 *
 * Separated from the DOM read so it can be tested: this decides what gets
 * written across an entire selection, and a blank leaking through as '' is
 * silent, plausible-looking data loss.
 */
function _invBulkPatch(fields) {
  const patch = {};
  const due    = String(fields?.due ?? '').trim();
  const terms  = String(fields?.terms ?? '').trim();
...
window._invBulkEditApply = async function() {
  ...
  const patch = _invBulkPatch({
    due:    document.getElementById('invBeDue')?.value,
    terms:  document.getElementById('invBeTerms')?.value,
    status: status,
    email:  document.getElementById('invBeEmail')?.value,
  });
